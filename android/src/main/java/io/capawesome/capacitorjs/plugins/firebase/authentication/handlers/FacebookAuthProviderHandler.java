...
public class FacebookAuthProviderHandler {
  ...
  public FacebookAuthProviderHandler(
    ...
    try {
      ...
      loginButton =
        new LoginButton(pluginImplementation.getPlugin().getContext());
  ...
  private void handleCancelCallback() {
    ...
    if (isLink) {
      pluginImplementation.handleFailedLink(
        savedCall,
        ERROR_LINK_CANCELED,
        null
      );
    } else {
      pluginImplementation.handleFailedSignIn(
        savedCall,
